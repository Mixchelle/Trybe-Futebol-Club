...
<h3>Banco de Dados 🗃️</h3>
<ul>
<li>Utilizei um container Docker MySQL configurado no docker-compose como serviço "db" para fornecer os dados ao backend.</li>
<li>Durante os testes, o Sequelize acessa o banco de dados por meio da porta 3306 do localhost.</li>
<li>É possível conectar a um cliente MySQL, como MySQL Workbench, Beekeeper, DBeaver, utilizando as credenciais configuradas no docker-compose para o serviço "db".</li>
</ul>
<h3>Backend ⚙️</h3>
<ul>
<li>O backend é responsável por implementar a lógica de negócio e fornecer os endpoints da API.</li>
<li>A aplicação backend roda na porta 3001 e é inicializada a partir do arquivo "app/backend/src/server.ts".</li>
<li>O Express é utilizado como servidor web e a aplicação ouve a porta definida nas variáveis de ambiente.</li>
<li>As dependências extras, como joi, boom, express-async-errors, são listadas no arquivo "app/backend/packages.npm".</li>
</ul>
<h3>Frontend 🖥️</h3>
<ul>
<li>O frontend já estava concluído e não foi necessário realizar modificações nele, exceto pela configuração do Dockerfile.</li>
<li>Os testes são executados utilizando o Puppeteer para simular a interação com o site em "http://localhost:3000/".</li>
<li>O frontend se comunica com o serviço backend através da URL "http://localhost:3001" e consome os endpoints disponibilizados.</li>
</ul>
<h3>Docker 🐳</h3>
<ul>
<li>O docker-compose é utilizado para unir os serviços backend, frontend e db em containers e subir o projeto completo com o comando "npm run compose:up".</li>
<li>Os arquivos Dockerfile nas raízes do frontend e backend são configurados adequadamente para inicializar a aplicação.</li>
</ul>
<h3>Criptografia de Senha 🔒</h3>
<ul>
<li>Utilizei a biblioteca bcryptjs para criptografar as senhas no banco de dados.</li>
<li>A biblioteca já estava instalada no projeto e não foi necessário alterá-la.</li>
<li>Explorei os recursos da biblioteca na documentação para implementar a criptografia ao cadastrar um usuário e realizar o login.</li>
</ul>
<h3>Requisitos e Respostas da API 📝</h3>
<ul>
<li>Segui as regras do protocolo HTTP e REST para definir os status e respostas da API.</li>
<li>Por exemplo, requisições que precisam de token, mas não o recebem, retornam o código de status 401.</li>
<li>Requisições que não seguem o formato esperado pelo servidor retornam o código de status 400.</li>
<li>Problemas inesperados no servidor retornam o código de status 500.</li>
<li>O acesso ao criar um recurso, como usuário ou partida, retorna o código de status 201.</li>
<li>Quando solicitado algo que não existe no banco, retorna o código de status 404.</li>
</ul>
<h3>Testes e Cobertura 🧪</h3>
...
<li>Configure corretamente os arquivos Dockerfile nas raízes das pastas frontend e backend do projeto.</li>
